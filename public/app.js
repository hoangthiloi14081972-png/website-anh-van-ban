...
function startRealtime(){
  if(realtimeStarted || !window.EventSource) return;
  realtimeStarted=true;
  const events=new EventSource("/api/events");
  events.addEventListener("posts-changed",()=>{ if(ME) loadPosts(); });
  events.addEventListener("comments-changed",e=>{
    if(!ME) return;
    loadPosts();
    const d=JSON.parse(e.data||"{}");
    const modal=document.querySelector("#modal");
    if(d.postId && modal && !modal.classList.contains("hidden")) viewPost(d.postId);
  });
  events.addEventListener("users-changed",()=>{ if(ME?.role==="admin") loadUsers(); });
  events.onerror=()=>{ /* EventSource automatically reconnects. */ };
}
